...
def load_and_process_data(file):
    ...
    for i, col in enumerate(cleaned_cols):
        if any(k in col for k in ['estado', 'estatus', 'fase', 'situacion']) and not any(ign in col for ign in ['comentario', 'observacion', 'detalle', 'descripcion']):
            estado_idx = i
            break
...
with tab1:
    ...
    with col_chart2:
        ...
        # Agregamos texto en las barras y ajustamos el alto dinámicamente
        ...
        fig_status.update_layout(
            ...
            height=max(350, len(df_st) * 35), # Ajuste dinámico de altura para que se vean todas las filas
